...
func TestPollAndPublishSSHLoginWritesOnlyValidatedEvent(t *testing.T) {
	...
	lastEventID := ""
	if err := pollAndPublishSSHLogin(context.Background(), manager, outputPath, &lastEventID); err != nil {
	...
	if event.ID != "cursor-1" || event.Username != "admin" || event.RemoteAddress != "203.0.113.9" || event.Method != "publickey" {
