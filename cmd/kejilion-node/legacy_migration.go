...
const (
	...
	lightSSHLoginEventPath   = "/run/kejilion-node-ssh/ssh-login.json"
)
...
User=root
Group=kejilion-node
ExecStart=/usr/local/lib/kejilion-node/kejilion-node ssh-login-broker --output ` + lightSSHLoginEventPath + `
...
func installLightNodeSSHLoginIntegration() error {
	...
	// binary. The service's Restart=always keeps retrying through that short
