...
func (c *Client) createDockerBackup(ctx context.Context) (string, error) {
	return c.createDockerBackupWithWalk(ctx, filepath.Walk)
}

func (c *Client) createDockerBackupWithWalk(ctx context.Context, walk func(string, filepath.WalkFunc) error) (string, error) {
	...
	walkErr := walk(sourceRoot, func(path string, info os.FileInfo, walkError error) error {
		if walkError != nil {
			return walkError
		}
		select {
		...
		if dockerBackupReservedTop(top) {
			if info.IsDir() {
		...
		if !dockerBackupTopPattern.MatchString(top) || top == "." || top == ".." {
			return errors.New("Docker backup contains an unsafe application path")
		}
		...
		written, copyErr := io.Copy(tarWriter, dockerBackupContextReader{ctx, io.LimitReader(file, info.Size()+1)})
		finalInfo, finalStatErr := file.Stat()
		closeErr := file.Close()
		if copyErr != nil {
			return copyErr
		}
		if written != info.Size() || finalStatErr != nil || finalInfo.Size() != info.Size() || !finalInfo.ModTime().Equal(info.ModTime()) {
			return errors.New("Docker backup source changed while reading")
		}
		return closeErr
	})
	...
	if budget.entries == 0 {
		return "", errors.New("Docker backup does not contain application data")
	}
	info, err := os.Stat(tempPath)
	if err != nil {
		return "", err
	}
	if info.Size() > maxDockerBackupBytes {
		return "", errors.New("Docker backup archive exceeds the 50 GiB safety limit")
	}
...
func dockerActionCompleted(action string) string {
	...
	case "backup_restore":
		return "Docker 应用文件已还原；归档中的同名顶层路径已替换"
	case "backup_migrate":
...
func safeDockerJobMessage(err error) string {
	...
	if errors.As(err, &recovery) {
		// root is generated by MkdirTemp under the configured application root;
		// never append raw multi-project errors to this bounded durable message.
		if recovery.applied {
			return "Docker restore applied; previous-data cleanup incomplete; inspect " + recovery.root
		}
		return "Docker restore/rollback incomplete; inspect current applications and retained previous project paths under " + recovery.root
	}
