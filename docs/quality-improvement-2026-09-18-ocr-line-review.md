...
- 可重复观察：`d028518a^..cc1e4118` 上 OCR v1.12.5 委托模式约束臂 6 条；自由臂在知晓这 6 条的前提下新增 7 条，并集 13 条，重合 46%；
...
| C 委托模式按风险自动参与（L2/L3 或 ≥30 行代码的 L1；挂在候选交付与质量审计流程，非阻塞提醒）+ 自由臂必做 + 试用退出 | 无需人工引导、零新门禁、小改动不增成本、可退出 | 达到阈值的代码候选增加一次评审成本 | 采用 |

## 最小改动方案

- 唯一入口：`scripts/ocr-delegate.mjs`（pin 读取、仓库外安装、禁更新、子命令与参数白名单、隔离 HOME、剔除 `OCR_*`/`OTEL_*`）。
...
- 复核人 / 智能体：独立干净 Claude 会话（未参与编写；限制：同一提供商，可能共享盲区）。
- 是否独立读取原始证据：是（run-0 EXPERIMENT.md、上游 v1.12.5 源码与文档，并实跑入口与治理核验）。
- 假设与方案评审结论：PASS WITH FOLLOW-UP，无 HIGH。M1 默认自动参与证据不足、比例性差 → 改为按风险
