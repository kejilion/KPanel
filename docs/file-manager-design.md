...
- 文档：PDF。
- 媒体：浏览器支持的音频和视频。
- 压缩包：ZIP、TAR、TAR.GZ/TGZ 可直接打开只读目录、进入子目录和搜索；勾选成员后保留包内相对路径解压，也可整包解压或继续下载。旧 Agent 未提供归档任务接口时保留原同步压缩/解压入口。
...
- 界面复用文件列表、右键、多选工具栏、标准弹窗和语义颜色。默认目标为当前目录，解压默认新建同名目录；
  已有结果不覆盖、不合并。批量解压每个包各建目录，逐项保留成功与失败结果。
...
- 关闭窗口或断开请求不取消已接收任务；重新打开文件管理可读取结果，任务中心从 Agent 同一任务源展示
  状态。停止后保留已完成结果。重试需重新读取源版本、核对目标和名称；只处理尚未完成的来源并保留成员选择。
