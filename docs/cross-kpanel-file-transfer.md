...
| 能力 | 当前实现与边界 |
| --- | --- |
| 主机入口 | 同一文件窗口支持本机、完整 Panel、轻量节点；按节点能力开放操作，失联时不回退到本机 |
| 日常文件操作 | 浏览/搜索/排序、多选、上传下载、预览编辑、复制移动、重命名、权限、回收站；具体权限由目标 Agent 校验 |
| 大批量操作 | 归档后台任务、受限 ZIP 下载；远程下载和分享继续依照各自节点能力，不因文件管理可用而推断全部支持 |
| 主机间复制 | 文件窗口拖放、跨主机复制粘贴、工具栏/右键“复制到其他主机”进入统一后台批次；跨主机移动不开放 |
| 桌面互传 | 桌面接收方沿用前台流式接口，成功后创建快捷方式；文件窗口接收桌面来源时可进入后台任务 |
| 稳定性 | 复用二进制流心跳、空闲/总时限、认证结束记录、Agent 完成校验与原子导入；目录容量为内容 10 GiB、10,000 项，普通文件 512 MiB |
| 结果可见性 | 展示来源、目标、阶段、字节/速率、逐项成功/失败及真实目标路径；刷新与关闭文件窗口不取消已受理任务 |
| 故障恢复 | 主动取消；重启后标记中断；仅未启动或确定未写入的项目允许显式重试；不做自动续传或不确定结果重放 |

数据路径仍为 `浏览器控制 -> 中心 Panel 调度 -> 来源 Agent -> 中心流式转发 -> 目标 Agent`。
文件内容不落到 Panel 的任务存储；主机选择、目标目录、来源路径和 `resourceVersion` 在提交时冻结。
每项执行前重新检查主机能力与授权；复制期间已授权的单项仍遵循现有流的取消/超时语义。
文件窗口关闭只停止观察。需要停止复制时使用任务的“取消传输”。

新增 `GET/POST /api/v1/files/transfer-jobs` 和
`POST /api/v1/files/transfer-jobs/{id}/cancel|clear`。创建请求包含 32 位小写十六进制 `id`、
`sourceNodeId`、显式 `targetHostId`（本机为空）、规范化 `targetDirectory` 和最多 64 项
`items[{path,resourceVersion}]`。仅已登录管理员可访问，写请求校验 Origin/CSRF 并记录审计。
同一 ID、同一请求体返回已有任务；丢失 HTTP 回执后先查询该 ID，不直接再复制。
`retryOf` 仅接收父任务允许恢复的完整子集，并在一次原子更新中消费恢复资格、创建子任务；
重复重试返回已有子任务。结果不确定的项目要求先检查目标目录。

后台同时最多 2 批执行，运行与排队合计最多 8 批，每批逐项执行、总时限 2 小时；
单项沿用传输核心时限。中心任务索引为 `${DataDir}/file-transfers/jobs.json`，Schema 1，
最多 32 批、8 MiB，终态保留 7 天；新建/重启清理过期记录，查询不返回过期终态。
目录 `0700`、文件 `0600`，临时文件同步后原子替换；进度最多每 5 秒持久化一次，
连接/完成/取消等状态转换立即持久化。无内容、密钥、Session 或 Cookie。
损坏状态保留原文件并拒绝新任务；写入失败不启动新批次，运行批次中止，恢复存储后重启并检查目标。
备份仅需在停止 Panel 后复制该有界目录；恢复后活动状态一律变为中断，旧版本可忽略此独立索引。
清除记录不删除目标文件，也不恢复已消费的重试资格。单 Panel 进程为唯一写者。

前端多个文件窗口共享一次查询：活动批次 2 秒、空闲 15 秒；后台标签暂停轮询，连续失败
退避 4 次后等待刷新/重新聚焦。完成通知携带目标主机，只刷新该主机对应目录。
路由切换与主机选择器共用不可取消写操作保护，已具有请求代次隔离的同主机路径传输保持原有取消语义。

产品复核（2026-09-13）：[1Panel v2 更新记录](https://1panel.cn/docs/v2/changelog/)
列出了文件上传总进度、远程下载取消和多节点切换优化，支持将可观测进度、取消和主机上下文一致性
作为本轮体验下限；不据此推断其跨主机复制实现、吞吐或安全水平。KPanel 继续复用现有
Panel/Agent 与 Noise 传输，不引入 SSH 文件守护进程或无界同步服务。
...
- KPanel A 到 B 的移动、双向同步、断点续传、面板重启后自动重放未完成的写入。
- 未配对面板与 Cluster v1 文件传输；轻量节点支持现有已授权的文件通道。
- 桌面接收方跨刷新恢复快捷方式创建；当前后台恢复范围为文件管理窗口接收的复制任务。
