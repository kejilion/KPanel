...
四列图标外框占五列，左右均匀留白；高度将标题与内容一起向整行对齐，上下平衡留白。默认指标下左右为 47.5px、上下为 50px，邻接外部图标/插件/展开组的水平间距为 5px、垂直为 4px。展开组锚点及拖动预览吸附桌面网格，旧锚点仅派生对齐、不在加载时回写；碰撞时保持原位置，不推开其他项。收起保留 56px 紧凑高度及原像素边缘吸附。主动留出的格位不压缩。桌面固定每逻辑行 4 个，旧组读取时保留格位并按四列显示，后续操作按四列保存；窄屏仍折行适配。不设置预留行数、列数或最大图标数量；已有存储容量边界不变。
