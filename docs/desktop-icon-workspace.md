...
显式空格不自动压紧，隐藏/暂离线成员占位保留。`rows` 为 0–8 的预留最少行数，0 表示随内容扩展；
已有远端格位需要更多行时自动长高。窄屏将逻辑行折行显示，不覆盖宽屏格位。

桌面空白右键可新建空组，多选工具条和图标右键可“编为一组”；拖到散放图标附近连续停留 450ms
显示“松开创建分组”，放下后才保存，快速经过、离开或 Esc 不成组。图标可拖入、拖出；
拖入空格保留其他空位，单图标在组内拖到占用格时交换，跨组加入时将被占格成员移至后续空格。
菜单加入优先填空位；Ctrl/Command+方向键提供逐格移动。拖动标题移动整组，标题按钮
收起/展开，收起条按格位顺序显示前 3 个可见成员的缩略图，窄屏减少缩略图但保留名称与操作。
分组底色与文字跟随应用浅深主题。绘制与拖放命中共用紧凑内容边界，左右和底部格位留白为 8px；
碰撞布局也使用实际内容边框，组锚点保留像素精度；相邻组边缘在 24px 范围内吸附至 12px 间距或对齐边缘，横纵方向一致，拖动预览与落点共用计算。冲突且无法就近吸附时保持原位置，不推开其他项；普通图标和插件仍按原网格移动。主动预留的行数及空格不压缩。桌面固定每逻辑行 4 个，旧组读取时保留格位并按四列显示，后续操作按四列保存；窄屏仍折行适配。更多菜单仅修改名称/预留行数或解散，不设置列数或最大图标数量；已有存储容量边界不变。
...
底板成组时轻微缩放淡入，解散时约 200ms 淡出；退场底板立即 inert 且不截获点击。确认后立即收起弹窗展示过渡，失败恢复布局及表单；减少动画偏好禁用过渡。
