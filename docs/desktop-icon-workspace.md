...
分组底色与文字跟随应用浅深主题。绘制与拖放命中共用紧凑内容边界，左右和底部格位留白为 8px；
碰撞布局也使用实际内容边框，组锚点保留像素精度；相邻组边缘在 24px 范围内吸附至 12px 间距或对齐边缘，横纵方向一致，拖动预览与落点共用计算。冲突且无法就近吸附时保持原位置，不推开其他项；普通图标和插件仍按原网格移动。主动留出的格位不压缩。桌面固定每逻辑行 4 个，旧组读取时保留格位并按四列显示，后续操作按四列保存；窄屏仍折行适配。不设置预留行数、列数或最大图标数量；已有存储容量边界不变。
