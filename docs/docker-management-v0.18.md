...
| 环境 | 备份/还原/迁移 | 后台归档 `/home/docker` 到 `/home/docker/.kpanel-backups`；还原完整校验后替换归档中的同名顶层项目，迁移只用既有 SSH 密钥与 known_hosts |
...
- 备份还原后的项目文件、数值属主、权限、`appno.txt` 和端口标记会回到脚本原布局；
  归档中的同名顶层路径（包括 KPanel 项目）整体替换，不在归档中的项目保持原状。
  根目录可以是指向真实数据盘的符号链接，归档内的链接仍不接受。
- 这是文件级备份，不暂停容器、不创建数据库快照，不保证运行中数据库或跨文件事务一致性；
  不改变 `kejilion.sh` 自己的容器备份包/还原脚本格式，也不宣称两种归档格式可互换。
...
- 还原先完整校验 tar 与 gzip 校验尾部，再将待覆盖项目移动到同一应用根的私有
  `.kpanel-restore-rollback-*` 目录，逐项复制归档内容。复制失败或取消时逆序回滚；
  目标删除失败时不继续回迁，旧目录回迁失败时不删除旧副本，其他项目继续各自回滚。
  已回迁或尚未处理的项目保持原字节。保留目录从后续备份中排除，也不能作为归档应用路径导入。
- 回滚未完成时任务为 `failed`，既有持久化 `message` 完整保存需要检查的恢复目录；项目以该目录下
  的原顶层名称定位。仅受控目录绕过普通错误的 400 字节摘录，底层长错误不会直接写入消息。
  内容已应用但旧副本清理失败时明确报告清理未完成。重读/Agent 任务 registry 重载不重放恢复动作；
  异常进程中断沿用 `failed/interrupted`，需要检查实际目录，不声称已经自动回滚。
...
- 创建和还原共享单文件 10 GiB、tar 载荷合计 50 GiB、100000 条实际可读 tar 条目的上限，
  目录条目计数但目录的文件系统大小不计入载荷；压缩归档同样不得超过 50 GiB。
  创建超限、源文件身份/大小/修改时间变化、取消或无可恢复数据时不发布正式归档，并移除临时文件。
  还原继续拒绝符号链接、硬链接、设备文件、路径穿越和不支持的顶层名称。
