...
| `system-virus-scan-adapter` | 系统中心 → 登录与安全 → 病毒查杀 | `KJ_VIRUS_SCAN_NONINTERACTIVE=1 k kpanel virus-scan <probe|update-db|scan>` 与 `KPANEL_VIRUS_SCAN_PROTOCOL_VERSION="1"`；数据库和扫描镜像沿用脚本现有 `clamav/clamav-debian:latest` 来源，报告固定为 `/home/docker/clamav/log/scan.log` | KPanel 只提交 `full`、`important`、`custom` 与最多 8 个规范绝对路径；Agent 复用单槽维护任务，扫描目标只读挂载且扫描容器禁网，读取有界报告，不复制 ClamAV 执行逻辑，不自动删除或隔离文件 | **成对本地候选，未发布**：`scriptLinkageState=coupled`，变更集 `virus-scan-20260920`；配套脚本候选 `kejilion/sh@90829558f42563b38c0d89ac299b4e19abde7af6`，根脚本 SHA-256 `de714f205e4dcec8ab576586cd896196f072fd200492f529c5d3d56f07245f2e`。正式合入或发布前必须先把脚本候选发布到权威 `main`，再更新 KPanel `Dockerfile` 固定提交与摘要，并完成 Linux/Docker/ClamAV 实机扫描和恢复验收；当前 KPanel 内置旧脚本会使写 capability 明确保持未就绪 |
