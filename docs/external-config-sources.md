...
`light-batch-enrollment-20260914`，两者均为 `scriptLinkageState=coupled`。配套脚本候选
`4d61f7ef123fe5ecd7419cdaa1c93483bbfda403` 已推送到权威候选分支，根/CN 同步与自动化回归已通过，
KPanel 也已用该精确 revision 重新生成 `cmd/kejilion-node/update_runtime/source.json`。发布 KPanel 前还须
复核公开短入口、最终摘要和 L3 成对证据；预览版不将脚本候选提升到 `kejilion/sh main`。
