...
| `managed-script-runtime` | SSH 端口、SSH 防御、DNS、BBRv3、应用、建站、体检、LDNMP 环境管理、系统资源、一条龙系统调优、磁盘与分区管理及轻量节点安装共同使用的宿主机脚本入口 | `kejilion/sh@2b90b2d2ca56bc954c9328a51bb5571e896f713d`；SHA-256 `806b4715664fad502f7faeccbc75972f2d1a24559d46208221b98f774ef56c99` | 镜像构建时按提交和摘要下载到 `/release/kejilion.sh`；安装/更新后以 root:root、0700 保存到 `/home/docker/kpanel/bin/kejilion.sh`，只继承既有可信脚本已明确接受的许可及区域、统计设置。应用市场仍优先使用受管脚本；仅当它不包含动态新增的内置 selector 时，才使用通过 root 所有权、写权限、协议和 selector 校验的宿主机 `/usr/local/bin/k`。并行原生脚本任务通过脚本并发协议保护共享的软件包、防火墙、应用目录和安装标记；磁盘写入仅在脚本包含 `KPANEL_DISK_MANAGEMENT_PROTOCOL_VERSION="1"` 时启用；第三方应用继续由受管脚本刷新动态配置；其他固定机器协议不变 | **已合规（代码链路）**：安装、旧版升级、摘要拒绝和回滚由生命周期测试覆盖；并发协议的锁释放、嵌套调用、失败传播、共享写入互斥和路径拒绝由脚本测试覆盖；磁盘协议只接收固定动作、可信 `MAJ:MIN` 与有界参数，事务、回读和恢复凭据由脚本负责；一条龙调优分离原生日志与机器回执，Swap 按 `/swapfile` 文件、激活状态和 `fstab` 回读，自动 DNS 直接复用既有固定 DNS 协议，新机 SSH 项只在一条龙中补齐 OpenSSH Server 和 systemd socket/service 前置条件；一条龙联网下载及其外部脚本执行显式清除代理环境并保持直连，避免宿主机代理变量污染特权调优；轻量节点新增 SSH 登录通知和文件代理的 systemd/OpenRC 生命周期、兼容回执与卸载清理；重新接入用 token 指纹和 root-only 暂存区原子切换身份，失败时保留旧连接，成功后按新 token 与指定名称完成上报；本组合新增 HTTP 站点端口协议、备份中心协议和病毒扫描协议；发布时必须先发布脚本提交，再构建 KPanel，并复核 OCI 标签与镜像内摘要 |
...
| `system-virus-scan-adapter` | 系统中心 → 登录与安全 → 病毒查杀 | `KJ_VIRUS_SCAN_NONINTERACTIVE=1 k kpanel virus-scan <probe|update-db|scan>` 与 `KPANEL_VIRUS_SCAN_PROTOCOL_VERSION="1"`；数据库和扫描镜像沿用脚本现有 `clamav/clamav-debian:latest` 来源，报告固定为 `/home/docker/clamav/log/scan.log` | KPanel 只提交 `full`、`important`、`custom` 与最多 8 个规范绝对路径；Agent 复用单槽维护任务，扫描目标只读挂载且扫描容器禁网，读取有界报告，不复制 ClamAV 执行逻辑，不自动删除或隔离文件 | **配套脚本已发布，KPanel 候选待 L3**：`scriptLinkageState=coupled`，变更集 `virus-scan-20260920`；配套脚本已先发布到权威 `main`：`kejilion/sh@2b90b2d2ca56bc954c9328a51bb5571e896f713d`，根脚本 SHA-256 `806b4715664fad502f7faeccbc75972f2d1a24559d46208221b98f774ef56c99`。根/CN 同步、协议 smoke 及隔离 Linux/Docker/ClamAV 1.5.4 真实病毒库更新和只读扫描均通过，样本前后摘要一致且测试资源已清理；KPanel `Dockerfile` 已固定该精确提交与摘要，回滚点为上一受管脚本 `6ebb945f6d5cb69fdb41e3761de23566acbaf762`。 |
| `system-package-manager` | 系统中心 → 日常维护 → 常用软件包 | `kejilion/sh@1f31dd885ea6887d7f9c42fe5e5643412b89d706` 的 `linux_tools()` 仅作为目录来源；KPanel 固定保留其中 17 项实用工具并排除 5 项屏保/游戏，实际状态和写入使用宿主机原生 APT、DNF/DNF5/YUM、APK、Pacman 或 Zypper | KPanel 只接收固定包 ID、安装/卸载动作和资源版本；每个包为独立后台步骤，首项失败即停止；交互工具通过固定 ID 新建本机独立 PTY 会话，不接受任意包名或命令，也不改动主终端页面的会话 | **本地候选，未发布**：`scriptLinkageState=not-required`；不修改脚本协议、运行时、安装路径或内置脚本摘要，OpenCode 的外部在线安装器明确排除在原生包事务之外；正式准入仍需 Linux 实机覆盖各发行版包可用性与部分失败状态 |
...
| `cluster-light-node-runtime` | 集群 → 添加主机 → 非面板 Linux 主机（单台或批量接入） | `bash <(curl -fsSL https://kejilion.sh) kpanel node join <授权>`；单台使用 `kpl1`，批量使用 `kpb1` 且每台机器保存独立 root-only attempt 身份用于幂等续接；官方短入口按区域加载 `kejilion/sh` 的根目录或 `cn/kejilion.sh`，二者保持同一节点协议；二进制与 `SHA256SUMS` 来自 `https://github.com/kejilion/KPanel/releases/latest/download/` | `kejilion.sh` 固定安装协议按架构下载静态 `kejilion-node`，严格校验 Release 摘要与 `version` 后原子安装；systemd timer 或 OpenRC hourly periodic 使用同一更新器自动更新，并在健康失败时回滚 | **已实现（成对候选待 L3）**：`scriptLinkageState=coupled`，变更集 `light-batch-enrollment-20260914`；配套脚本已先发布到 `kejilion/sh@4d61f7ef123fe5ecd7419cdaa1c93483bbfda403`，根脚本 SHA-256 `a0e6cf5193aa9bfbdc572ad91803d4b53d34549ac95259b3288d436766a229dc`，CN 脚本 SHA-256 `132b38bf1300e529ca88e365d075be599ef3466ac1aece48bce6ce7565f344d0`。GitHub 原始入口和国内代理短入口均已返回新协议；正式发布须继续验证 Release 资产、断网重试、更新回滚及真实 OpenRC PID 1 的重启恢复 |
| `cluster-light-node-update-migration` | 既有轻量节点自动更新恢复 | 配对版本固定到 `kejilion/sh@4d61f7ef123fe5ecd7419cdaa1c93483bbfda403`，完整根脚本 SHA-256 `a0e6cf5193aa9bfbdc572ad91803d4b53d34549ac95259b3288d436766a229dc`；模板摘要与该精确 revision 已同步到 `cmd/kejilion-node/update_runtime/source.json` | `scripts/sync-light-node-runtime.mjs` 原样组合安装器共用生命周期锁与 updater，并提取 systemd service/timer、OpenRC periodic 与 SSH login service 后嵌入节点；普通 Go 测试核对模板摘要，配对验收执行该脚本 `PATH_TO_KEJILION_SH --check`。官方历史 unit 精确匹配后同源迁移，自定义 unit 与 drop-in 保留 | **已实现（候选待 L3）**：配置权限、锁/交接、下载失败、回滚、进程版本、systemd/OpenRC 命名空间迁移有回归；隔离 Ubuntu 24.04 的真实 systemd 历史证据继续有效，新增 OpenRC 仅有自动化契约证据。配套脚本已在权威主线按精确 revision 可用；正式发布须完成真实 Alpine/OpenRC PID 1、公开 Release 资产和最终 SHA 验证 |
| `cluster-notification-channels` | 集群 → 通知；Telegram、飞书、钉钉、企业微信 | `kejilion.sh` 无 KPanel 集群通知配置或发送协议；脚本中的独立 Telegram/OpenClaw 配置不是本业务权威来源 | KPanel 中心端原生发送；只允许 `api.telegram.org`、`open.feishu.cn/open-apis/bot/v2/hook/*`、`oapi.dingtalk.com/robot/send?access_token=*` 和 `qyapi.weixin.qq.com/cgi-bin/webhook/send?key=*`，统一超时、禁止重定向、限制响应体并校验成功结构 | **已合规（KPanel 原生代码与本地隔离契约，真实平台待 L3）**：消息渠道本身不依赖 `kejilion.sh`；交互和平台请求格式参考 `huilang-me/CF-Server-Monitor@1dc0dc49b0e954fa325f1a95f88eaaa1c8fab8bf`，但不在运行时依赖其代码或外部配置；正式发布前须用临时可撤销凭据逐平台验收并立即轮换 |
...
当前轻量节点候选包含 Alpine/OpenRC 变更集 `kpanel-openrc-alpine-v1` 与批量接入变更集
`light-batch-enrollment-20260914`，两者均为 `scriptLinkageState=coupled`。配套脚本
`4d61f7ef123fe5ecd7419cdaa1c93483bbfda403` 已先发布到权威 `main`，根/CN 同步、公开短入口和自动化回归已通过，
KPanel 也已用该精确 revision 重新生成 `cmd/kejilion-node/update_runtime/source.json`。发布 KPanel 前还须
复核最终摘要和 L3 成对证据。
