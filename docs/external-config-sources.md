...
| `system-hosts` | 概览页本地 hosts 管理 | `kpanel_hosts_noninteractive` | Agent 读取 `/etc/hosts` 事实；写入仅传递结构化记录给可信脚本 | **已验证**：154 完成新增、回读、删除与现场恢复 |
| `system-cron` | 概览页 root crontab 管理 | `kpanel_cron_noninteractive` | Agent 读取 Debian/Red Hat root cron；写入由可信脚本精确执行 | **已验证**：154 完成新增、回读、删除与现场恢复 |
| `system-network-interface` | 概览页网卡管理 | `kpanel_network_noninteractive` | 接口名结构化校验后调用 `ip link` 并回读 | **已验证（隔离）**：154 隔离网络环境完成 up/down，不触碰生产网卡 |
| `system-firewall` | 概览页防火墙管理 | `kpanel_firewall_noninteractive` | 固定动作调用脚本原生规则并统一快照、验证、持久化和失败回滚 | **已验证（隔离）**：154 完成端口、PING、DDoS、全开/全关、SSH 保留及注入失败回滚 |
...
| `managed-script-runtime` | SSH 防御、DNS、hosts、cron、网卡、防火墙、BBRv3、应用、建站、体检、LDNMP 环境管理与轻量节点安装共同使用的宿主机脚本入口 | `kejilion/sh@193a29710d18421242b2d5e2358ff36543e360cf`；SHA-256 `7eb40d270323e91d12ce989ff13baa5b16f0feb53ef2906741d87eefcc3bb6d6` | 镜像构建时按提交和摘要下载到 `/release/kejilion.sh`；安装/更新后以 root:root、0700 保存到 `/home/docker/kpanel/bin/kejilion.sh`，只继承既有可信脚本已明确接受的许可及区域、统计设置 | **已合规（代码链路）**：154 脚本协议通过；发布镜像仍须复核 OCI 标签与镜像内摘要 |
