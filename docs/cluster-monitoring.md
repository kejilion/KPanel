...
普通 KPanel v2 与轻量文件 broker 默认使用 `GET /api/v2/federation/files/stream`，
WebSocket 子协议为 `kpanel-file-stream-v1`。每个文件请求独占一条双向连接；首帧只协商角色，
通过既有静态密钥的 Noise IK 认证后才发送固定文件请求。握手 prologue 绑定 GET、路径、双方 ID、
时间戳与随机 request ID；后续内容使用独立收发 CipherState，二进制数据每帧最多 60 KiB，
不压缩、不逐块重新握手，不将文件字节转换为 JSON/base64。目录、编辑、上传、下载、归档、
跨节点导出与导入复用同一文件请求协议；反向 linked grant 仍只允许 GET 导出，不获得文件管理权限。

轻量节点保持一条主动连向中心的控制连接，收到请求 ID 后再主动建立该请求的数据连接。
回接必须匹配节点身份、当前控制连接 generation、未过期且未消费的请求 ID。
控制连接每 15 秒心跳，45 秒无响应断开，按 1/2/4/5 秒退避重连；不重放已经开始的文件动作。
普通 Panel 保留现有 DNS/IP 拨号检查、TLS 验证及禁止重定向，轻量节点保留环境代理支持。
使用反向代理时，该精确 GET 路径需要允许 WebSocket Upgrade，并容纳 15 秒心跳间隔。

每端分别为大文件与目录/元数据操作保留全局 16、每身份 4 个并发；轻量 broker 最多 8 个数据任务。
中心另外限制 64 个未认证握手、128 个已认证连接和每身份 10 个连接（包含控制连接）。
背压直接传到读写端，文件握手总预算 15 秒；普通 Panel 文件连接独立使用 8 秒 TLS/响应头预算，
不修改监控 HTTP 的 3 秒预算。数据连接每 15 秒发送 WebSocket Ping，仅收到 Pong 才续连接活性，
允许目录扫描和文件操作等待结果；上传无进度或读写背压仍在 45 秒内终止，单文件最长 2 小时。
控制连接不受单文件期限影响。两端都更新后才能消除旧数据连接的 45 秒静默断开；混合版本仍受旧端限制。
单文件上传上限仍为 512 MiB，未知长度上传同样受限。仅目录导入使用独立有界 TAR 预算：
内容最多 10 GiB、10,000 项，另留每项 16 KiB 和结束块 1 KiB 的 TAR 元数据空间。
只有加密结束帧的累计长度匹配才产生 EOF；
导出还必须通过 Agent 的最终 `X-KPanel-Transfer-Result: ok` 校验。取消、断流、撤权、删除节点和
Service 关闭都会取消对应连接、请求和管道；中途失败不会转换为干净 EOF 或自动重试写操作。
Panel 为认证失败及每个文件连接的完成/失败保留审计，控制心跳不逐条写审计。
普通节点的 Agent 上传/导入在写结果时刷新响应期限，避免持续上传超过默认两分钟后已落盘却无法回执。
下载和 ZIP 的上游读错会中断输出，不能形成成功结束帧。远端操作限流返回完整的加密 429 响应；
跨桌面复制也检查本地来源的导出 trailer，并在读取目标完整响应及认证结束后才报告完成。
连接失败区分认证、TLS、反向代理拒绝、超时和断流，不显示原始代理响应或敏感地址。
普通节点页面显示“已授权文件管理”，表示配对授权存在，不把授权状态当作当前连接成功。

只有 GET 升级前明确返回 404/405/426 才兼容旧协议；101 后的认证失败、断流、超时均不回退。
轻量 broker 选中兼容模式后维持到进程重启，避免探测升级时中断旧传输；中心升级后可通过节点正常更新/重启切入新协议。
中心不会仅因旧 poll 活性缓存而降级；节点回滚旧版本后新到达的认证文件 poll 可在流式控制已断开时恢复兼容模式。
v1 Panel 保留原有 HTTPS 流式接口，旧 v2 Panel 和旧轻量节点保留以下轮询实现。

兼容文件 broker 继续复用既有 Noise 身份、固定文件路由和 64 KiB 业务负载，不增加节点入站端口。
...
| 最大并发 | 普通摘要轮询 8、单主机连接 2；每个轻量节点最多 4 个终端会话；文件流按大文件/短请求分别全局 16、每身份 4 个，升级连接单独限额；请求与 nonce/rate-limit 缓存均有界 |
| 超时与重试 | 普通联邦连接 2 秒、响应头 3 秒、总计 6 秒；轻量终端长轮询最长 25 秒，节点请求总超时 40 秒；轻量文件命令接单最多 12 秒；节点首次收到旧中心 404/405/426 时从 1 秒指数退避至 5 分钟，其他文件连接失败按 1/2/4/5 秒退避；Relay 丢失确认时可重发同一命令和完全相同的数据批次，浏览器不重复请求，写入不自动重试 |
...
- 普通与轻量节点真实 TLS 文件流、上传/下载/Range、跨节点复制与反向只读导出；提前拒绝、取消、帧篡改、缺失结束帧/最终 trailer、撤权及旧端点回退；慢大文件期间目录保留并发容量；
