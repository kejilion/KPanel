...
| 超时与重试 | 普通联邦连接 2 秒、响应头 3 秒、总计 6 秒；轻量终端长轮询最长 25 秒，节点请求总超时 40 秒；轻量文件命令接单最多 12 秒；节点首次收到旧中心 404/405/426 时从 1 秒指数退避至 5 分钟；文件读取丢失响应时仅由 Relay 重发相同命令，浏览器不重复请求，写入不自动重试 |
