# KPanel 集群通知设计与实现边界

- 状态：Telegram 首个通道已实现；真实 Bot API 联通、隔离真机和正式发布仍按本节验收边界执行
- 入口：`集群 → 通知`
- 目标：在不引入常驻队列、数据库或额外 Agent 权限的前提下，让一个 KPanel 统一通知本机和全部已接入主机

## 1. 设计取舍

通知是 KPanel 控制端的派生能力，不建立第二份主机资源事实：

- CPU、内存、磁盘、收发流量直接消费既有 `cluster.HostList` / `HostSnapshot`；
- 掉线沿用集群现有 `online`、`degraded`、`stale`、`offline`、`auth_failed`、`tls_error`、`incompatible` 状态机；
- SSH 登录只新增窄字段 `HostTelemetry.SSHLogin`，不传递原始认证日志、SSH 配置或凭据；
- 配置使用有界 JSON，Bot API key 与状态分文件保存；不引入 Redis、SQLite 或消息队列；轻量节点的
  SSH 读取只增加受限 root broker，不下发 Bot 凭据，也不让遥测进程获得 root 权限。

Telegram 接入只要求用户输入 BotFather 生成的 API key。KPanel 通过 Telegram Bot API `getMe` 校验机器人，
再读取有限的 `getUpdates`，自动寻找用户私聊机器人后发送通知；用户不需要输入 `chat_id`。第一次接入时，
用户需要先私聊机器人发送 `/start`。如果机器人正在使用 webhook，界面会明确提示，不会静默覆盖 webhook。

## 2. 交互

集群页 Hero 区只增加一个“通知”入口，打开一张轻量弹窗：

1. Telegram 通道卡片显示未配置、等待私聊、已连接或需要检查；不显示 token 和 `chat_id`。
2. API key 输入框留空表示保留原值；输入新 key 后“保存并连接”会一次完成校验、发现私聊和落盘。
3. “重新发现私聊”用于用户补发 `/start` 后重新绑定会话；“发送测试消息”验证真实发送链路。
4. 总开关保留全部阈值与事件开关；关闭只暂停发送，不删除配置。
5. 资源规则使用启用复选框加阈值；累计接收和累计传送分别配置，事件规则只使用开关，不增加分散的高级配置。
6. 通知语言在保存设置时记录当前 KPanel 界面语言（简体中文、繁體中文或 English），之后不随其他浏览器或页面临时切换而漂移。
7. 消息时间使用通知中心这台 KPanel 宿主机的 Agent `management.timezone`，并始终附带明确的 `UTC±HH:MM` 偏移；
   读取失败时回退当前进程时区，存储时间仍统一使用 UTC。

所有集群主机（包含本机）复用同一组规则。界面说明连续 3 次采样才触发，恢复只发送一条恢复消息；未知、
无效或零值指标不产生资源告警。弹窗支持已有 `ModalDialog` 的 Escape、焦点回收、键盘 Tab、桌面/移动视口和
浅色/深色 semantic token。移动窄屏下“添加主机”单独占满操作区，避免被挤到下一行后变成无内容的矩形。

## 3. 告警语义

| 类别 | 条件 | 去重与恢复 |
| --- | --- | --- |
| CPU / 内存 / 磁盘 | 有效百分比达到阈值，默认 90% | 连续 3 次后告警；低于阈值发送恢复；持续告警最多每 6 小时重复 |
| 流量 | 收发总速率达到阈值，单位 MiB/s，默认关闭 | 首次使用集群已有速率，之后使用计数器差分；计数器回退或速率无效时跳过 |
| 累计接收 | 每台主机的累计接收字节达到独立阈值，默认关闭 | 与概览桌面监控小组件同源、同样使用 1024 进位和 1 位小数显示；达到后每个计数器周期只告警一次，计数器回退/重启归零后重新计数，不发送“恢复” |
| 累计传送 | 每台主机的累计传送字节达到独立阈值，默认关闭 | 与累计接收独立去重、独立重置和独立通知 |
| 掉线 / 失联 | 集群状态进入 stale、offline、auth_failed、tls_error 或 incompatible | 状态恢复到 online/degraded 后发送恢复；单次评估最多发送 8 条，失败 5 分钟后重试 |
| SSH 登录 | 出现新的、通过校验的 `SSHLoginEvent.ID` | 首次观测只建立基线；之后每台主机按事件 ID 去重，只发送用户、来源、方式、发生时间 |

累计接收和累计传送始终直接使用既有 `NetworkSummary.ReceivedBytes` / `SentBytes`，不再创建收发合计指标；
前端集群卡片、公开分享和概览桌面监控统一复用同一个方向格式化器，通知消息也按相同的 1024 进位和 1 位小数呈现。
这样调整收发任一方向的单位或精度时，只需要修改对应共享层并同步验证，不会出现各页面各算一套的情况。

Telegram 发送有 6 秒超时，后台评估默认 30 秒一次；所有网络调用均使用固定 HTTPS API 根地址、TLS 1.2、
有界响应体和最大并发/发送数。Telegram 失败只影响通知通道状态，不改变集群主机状态，也不阻塞浏览器读取。

## 4. 存储与恢复

数据目录下新增 `notifications/`：

- `notification-state.json`：开关、规则、通道状态、资源版本和最多 1024 个有界告警状态；权限 `0600`；
- `telegram-bot-token`：仅保存 API key，权限 `0600`，目录权限 `0700`；日志、审计和 API 响应均不包含 token/chat ID；
- 两个文件都采用同步、原子替换和 Windows 可恢复替换路径；启动时严格 JSON 解码并校验大小、字段和权限。

PUT、重新发现和测试均经过现有 Session、Origin/CSRF、审计和 `ExpectedResourceVersion` 冲突控制。新 token
先完成 API 校验和私聊发现，再写 secret；状态写入失败会恢复旧 token。服务重启后从告警状态恢复去重，
不会因为重启重复发送同一个 SSH 登录事件。

## 5. 兼容与协议边界

新的 `SSHLogin` 是可选字段，并通过能力头协商：新 v1/v2 控制端声明 `ssh-login-v1` 后，被控端才返回；
旧控制端仍收到旧形状摘要。本机 Panel 到 Agent 也只在带有 `capabilities=ssh-login-v1` 的遥测请求时读取该可选字段，
避免旧 Agent/新 Panel 升级窗口产生严格 JSON 解码冲突。

普通 Agent、轻量节点 root broker 和轻量节点遥测进程共用集群边界中的窄事件读取器。普通 Agent 直接读取
Linux journal，journal 可读但没有 sshd 记录时回退 `/var/log/secure` 或 `/var/log/auth.log`；轻量节点由
`kejilion-node-ssh-login.service` 以 root 读取相同来源，只把严格校验后的单条事件原子写入
`/run/kejilion-node-ssh/ssh-login.json`，非 root 遥测服务继续通过原有 `light-v1` HMAC 上报。
Telegram 发送仍只发生在中心端。

新中心在 enrollment 和 report 响应中返回 `X-KPanel-Light-Response-Capabilities: ssh-login-v1`。
新接入节点立即启用；已有轻量节点由现有 root 自动更新任务在下载并校验新版 Agent 时自动安装采集服务，
不需要逐台执行 `k kpanel node update`，也不需要再次配对或消耗一次性授权码。节点配置和 reporting key
保持不变；重启后最多先发送一次旧格式 report，再根据响应头启用 SSH 字段。旧中心不返回响应头时继续发送旧格式，
不会破坏兼容性；若旧中心因严格 JSON 解码拒绝一次带字段 report，节点会自动去掉 SSH 字段重试并回退到旧格式。
第一次看到已有登录记录时只建立基线，不会把历史记录当作新告警。

本阶段只实现 Telegram。交互层保留 Telegram、飞书、钉钉、企微四个渠道选项，但后三者明确标记为“暂未开放”，
不产生虚假的连接状态或配置契约；后续按同一通知通道边界逐个增加，避免每种通道引入独立后台系统。

## 6. 验收边界

### 本机证书与关键容器提醒

`集群 → 通知 → 本机资源提醒` 默认关闭，复用上述 Telegram、30 秒评估、连续样本、
单轮 8 次发送尝试、5 分钟失败重试和最多 1024 条持久状态。不增加调度器、资产库、
Docker 写动作或网站主动 HTTP/TLS 探测；通知不是公网可用性证明。

- 证书读取现有 Nginx 配置解析器发现的 TLS 绑定及 `TLS.ExpiresAt`。同一证书路径的多个
  网站绑定合并提醒；路径摘要是稳定 ID，公开证书摘要是换证指纹，网站名称仅供展示。
  剩余时间精确小于等于 30 / 7 / 1 天，以及小于等于零时分别进入阶段；首次开启只发送
  当前阶段，每证书/阶段一次。换证后的有效期重新判断，超过 30 天才解除到期提醒。
- 维护方式复用固定脚本已有 `<domain>.custom` / `.auto-renewal`，不新增标记。
  自有标记必须为可信路径下 root 所有、0600 的普通文件，内容为 `custom-v1\n`，且当前
  证书路径与公开指纹匹配；自动资格要求脚本 130 字节 proof 与当前证书和私钥摘要均匹配，
  自有标记优先。缺失、损坏、链接、权限/属主不可信或身份无法证明时为未知。
  自动资格不证明定时器健康或续期成功。Panel 只接收维护方式、名称、到期时间和公开证书
  指纹，绝不接收证书内容、私钥内容/路径或原始资格摘要。
- 容器使用 Docker 的完整稳定 ID，最多显式选择 64 个；不按名称重新绑定。连续三次有效
  新样本的 unhealthy、未运行、restarting 会提醒；首次观察只建立 RestartCount 基线，
  5 分钟窗口内观察到增量至少 3 才判断反复重启。计数器回退重新建基线。
  只陈述已证实状态，不猜测退出原因；手动停止仍显示未运行，绝不自动启动。
- 可关闭单个选择，或将本机资源整体/单个容器暂停 1 小时；后端允许的暂停上限为 24 小时。
  必须保存设置生效，到期自动恢复评估，并重新累计连续样本。暂停不发送故障或恢复。
- 读取失败、字段缺失、过期、未来时间或 health=starting 不能证明恢复；连续样本遇到未知
  或超过 90 秒间隔会中断。只有完整成功读取能证明资源删除；删除清理派生状态，不发送
  “恢复”。容器配置保留原 ID 供用户关闭；同名重建需要重新选择。改名及资源版本变化按
  原稳定 ID 继续观察，通知保存仍受 `expectedResourceVersion` 配置冲突检查保护。
- `rules.resourceAlerts` 为可选字段。旧配置默认关闭，旧客户端 PUT 省略该字段时保留
  已有资源规则；新 UI 对无资源字段的旧 API 不回传新增规则。新 Agent 窄接口为
  `GET /v1/notification-resources`，沿用 Agent Bearer 鉴权，仅接受无查询参数的 GET；旧
  Agent 返回 404 时显示未知，不扩展旧 `ContainerSummary` 或集群摘要协议。
- Agent 同时最多一个资源请求，8 秒总预算；Docker 最多 128 个列表条目、4 个并行 inspect，
  每个响应最多 256 KiB。证书扫描 `conf.d` 最多 128 个目录条目，复用现有配置/证书
  文件限额；超限显示 limited，不把截断结果当完整资产清单。Panel 响应上限 256 KiB，
  内存缓存 30 秒，多个弹窗共享一次读取；资源规则均关闭时评估器不额外采集，打开配置
  弹窗仍可读取以供选择。缓存不是持久资产事实。
- 状态满额先拒绝为新资源发送，保留已有状态与恢复/重试，并在 UI 明示容量；腾出容量后
  新资源才建立状态。主机与本机提醒共用 8 次尝试预算；超额待下一轮。持久去重跨重启保留；
  Telegram 成功而本地磁盘提交失败仍可能在重启后重复，不能承诺 exactly-once。

`scriptLinkageState=not-required`（无需发布脚本，不适用）：本实现只读取当前固定脚本
`298f6f23751e36726660d73b5c0c83aef1b404f4` 的既有产物，不改变脚本、签发、替换或续签协议。
旧二进制采用严格状态解码；回退到不认识本机资源规则的版本前，应备份通知目录，并用兼容
版本恢复升级前的通知配置，而不能直接让旧版本读取新增规则。Git 回退不自动回退运行数据。

定向回归覆盖阶段边界、未知、删除/改名、换证、计数器变化、禁用/暂停、旧客户端往返、
重启去重、容量、发送预算和失败重试；读取链以本地测试 HTTP 接收器、Unix Socket、
固定 Docker 响应及文件夹具验证。Mock UI 与这些集成夹具均不代表真实 Telegram 或生产验收。

- L0/L1：类型、能力协商、严格解码、token 脱敏、原子文件、状态机和前端类型/编译测试；
- L2：完整 `go test ./...`、前端构建/检查、`make verify-change` / `make verify-l2` 对应门禁；
- L3：仅在 `arena-154` 验证服务启动、配置目录权限、重启恢复、集群主机状态变化和真实 Agent/轻节点遥测路径；
- Telegram 真实发送必须使用用户临时提供的 Bot API key，验证后立即撤销或轮换，不保存到仓库、终端记录或交付物。

没有真实 token 时，Telegram Bot API 的 HTTP 交互使用本地假服务覆盖；这不能替代真实 Telegram、真实网络和
真实私聊发现验收，最终报告会分别标出两类证据。
