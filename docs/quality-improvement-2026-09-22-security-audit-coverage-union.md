...
- 提案状态：试行（独立复核两轮：PASS WITH FOLLOW-UP → 修复后 PASS；主线由发布任务集成）
- 提案日期：2026-09-22
...
- 是否独立读取原始证据：是；自建本地克隆与 11 个一次性仓库对跑新旧实现，只读取 run-4 的元数据文件
- 假设与方案评审结论：第一轮（f7953760）PASS WITH FOLLOW-UP：构造的全部反例中并集都没有把未审提交判为已审，
  理论上新实现认领范围不超过旧链；1 重要（验收字段接受任意 run-N，rc.9 原文可放行）、3 一般（合并提交引入的新包漏判、
  base 对象缺失时抛错、已删包误报）。修复提交 a0236571 后第二轮（c10af57d）只重验受影响项与冻结回归集，结论 PASS
- 门禁是否被削弱、绕过或只对样例优化：否；"什么算已审"未放宽，合并提交引入新包的检测面回退已在第二轮前修复
- 复核状态：通过
...
- 决策：继续试行（复核通过；候选 CI 与主线集成见交付记录）
- 后续事项：run-4 线索验证完成后按"成立的发现位于改动代码还是未改代码"评估全量间隔；`Security-Audit` trailer
  与豁免次数的统计入口（前序提案主指标）留待下一轮；验收字段只核对"提到的 run 是否 complete"，不核对它是否
  就是解决本次 decision 的那次（提到早已完成的 run-1 也会通过），由发布任务与独立复核核对；豁免截止日不超过 14 天
  未做机器校验。
