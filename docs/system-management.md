...
| 病毒查杀 | `/home/docker/clamav/log/scan.log` 与单槽后台维护状态 | 调用可信 `kejilion.sh` 的 ClamAV 固定协议；支持全盘、重要目录或最多 8 个规范绝对路径，先更新病毒库再扫描；目标目录只读挂载，扫描容器禁网，只生成有界报告，不自动删除或隔离文件 |
...
| 常用软件包 | 固定 17 项实用工具的可执行状态与软件包管理器 | 取自 `kejilion.sh` 基础工具清单并排除屏保/游戏；只接受固定 ID，支持批量安装、卸载和部分交互工具的本机终端启动 |
...
- 日常维护：常用软件包、系统更新、系统清理、系统日志和服务器重启；
- 基础配置：虚拟内存、磁盘与分区、主机名、时区、系统更新源和定时任务；
- 登录与安全：SSH 端口、SSH 防御、账户管理、防火墙和病毒查杀；
...
- 入口位于“系统中心 → 日常维护”，从 `kejilion.sh` 的“4. 基础工具”保留 17 项常用实用工具；
  `cmatrix`、`sl`、`bastet`、`nsnake`、`ninvaders` 等屏保/游戏不进入 KPanel 软件包目录。
- 已安装状态沿用脚本的“命令是否可执行”语义；页面只在打开管理器时读取，不给系统中心首页
  增加 17 次软件包查询。
- Web API 仅接受固定包 ID、`install`/`remove` 动作和 64 位资源版本；不接受任意包名、仓库、URL、
  路径、Shell 或包管理器参数。OpenCode 使用独立在线安装器，因此不进入本原生软件包事务。
- Agent 使用 APT、DNF/DNF5/YUM、APK、Pacman 或 Zypper 的固定非交互参数，把多选操作拆成单槽
  持久化后台步骤。首个失败项会停止，已成功的软件包保持真实状态；页面不会宣称整批事务可回滚。
- `htop`、`iftop`、`tmux`、`btop`、`ranger`、`ncdu`、`fzf` 安装后可“一键启动”。
  每次启动会新建独立的本机 PTY 会话，不占用或修改主终端页面的已有会话；前端只把固定软件包 ID
  映射到内置命令，URL 和 API 均不能注入命令。
- `scriptLinkageState=not-required`：本功能不调用或修改脚本协议、运行时、安装路径或 OCI 内置脚本；
  目录兼容性已对照 `kejilion/sh@1f31dd885ea6887d7f9c42fe5e5643412b89d706` 的 `linux_tools()`。
...
- 本功能不浏览、编辑或删除任意日志文件，不执行 `rm -rf /var/log/*`，不修改 `journald.conf` 或
  `logrotate`，不混入网站、Docker、应用、KPanel 审计或后台任务日志，也不引入远程聚合、全文索引、
  告警规则或自定义清理路径。

## 病毒查杀

- 入口位于“系统中心 → 登录与安全”，不增加概览首页快捷卡片。只有打开弹窗时才读取
  `/home/docker/clamav/log/scan.log` 的有界尾部；报告是宿主机事实，Panel 不保存第二份扫描明细。
- Agent 只接受 `full`、`important`、`custom` 三种模式。重要目录固定为 `/etc`、`/var`、`/usr`、
  `/home` 和 `/root`；自定义模式只接受 1–8 个已存在、规范且不含逗号或控制字符的 Linux 绝对目录。
- 写入复用现有单槽维护任务，后台依次调用
  `KJ_VIRUS_SCAN_NONINTERACTIVE=1 k kpanel virus-scan update-db` 和
  `... scan <mode> [paths...]`。浏览器关闭不会终止任务，也不能与系统更新、清理、日志清理或调优并发。
- 病毒库更新容器允许联网；实际扫描容器使用 `--network none`、只读根文件系统、丢弃 capabilities，
  并把每个目标目录只读挂载到固定容器路径。扫描结果为“发现威胁”时任务仍正常完成，便于管理员查看报告；
  只有执行或报告生成失败才标记任务失败。
- 页面最多读取报告尾部 256 KiB、展示 200 条威胁位置，单条最长 2048 字节，并明确标记截断。
  功能不会自动隔离、删除、修复或上传文件；管理员必须核对来源后另行处置。
- 该能力要求可信脚本同时包含系统资源协议和 `KPANEL_VIRUS_SCAN_PROTOCOL_VERSION="1"`。
  不满足要求时只显示适配器未就绪，不回退到 Panel 自编 ClamAV 命令。
