...
**这是安全要求，不是配置偏好，没填就不放行。**

Scramjet 的页面改写由 Service Worker 完成，被改写的第三方页面会在**浏览壳所在的那个 origin
下执行**。改写器是尽力而为的重写，不是安全边界；一旦被绕过，那个页面就拿到了该 origin 的真实
权限。如果它和面板同域，那就意味着：面板会话 Cookie 由浏览器自动附带（`HttpOnly` 只挡读取，
不挡发送），CSRF Cookie 按 double-submit 的设计本来就可被脚本读取，`Origin` 校验因为它的
Origin 就是面板的而自然通过——`/api/v1/files`、`/api/v1/docker` 全部敞开，等价于宿主机 root。

拆成两个 origin 后，这道边界落在浏览器内核里：面板的 Cookie 是 host-only，永远不会发到浏览域名；
从浏览域名回打面板的跨源请求会被预检拦下。被绕过的页面手里只剩一个浏览作用域的凭证，能做的
仅是它本来就能做的中继抓取。

配置方式（**设置 → 面板访问域名**，左栏是面板域名，右栏是浏览器专用域名）：
...
CDN 场景下（例如 Cloudflare），加一条子域记录指向同一个源站即可，通配证书通常已经覆盖。
...
这条出口，所以恶意页面上一行 `<img src="http://192.168.1.1/…">` 就等于让你的服务器从内网发起请求，
不需要点击，也不需要绕过改写器。

如果确实要用桌面浏览器访问本机或内网服务（本机的 NAS、另一个容器、只监听 `127.0.0.1` 的服务），
在**设置 → 面板访问域名**里勾选"允许浏览器访问本机与局域网"。注意：
