...
候选分支与发布后处置：`release/vX.Y.Z-candidate` / 稳定版归档、预览版保留、失败保留（按 `project-management.md` 10.2）

- 原分支 / 精确 tip / 处置分类：
- 归档 ref 与 SHA（或历史 tag/bundle 恢复证据）/ 远端复核结果：
- 本次来源任务分支：纳入版本/替代依据、归档或保留理由：
- 本地分支/upstream/worktree：已回收 / 保留及原因（不能以远端归档代替本地核对）：
- 未完成归档项 / 责任人 / 下次复核触发条件：
...
- 覆盖检查（`check-security-audit-coverage.mjs` 的 decision、未审计提交数；RC 只记录。稳定版带 `--require`，非 ok 时写补审 run，或用户原话、理由与不超过 14 天的补审截止日）：
...
- 变更集编号（跨仓库时必填；不适用时写“不适用”）：
- KPanel 实际内置脚本基线 commit / SHA-256：
- 脚本候选 commit / SHA-256（不适用时写“不适用”）：
- 状态判定依据与兼容性证据：
- 本版发布决定：脚本不在范围 / 脚本先行后发布 KPanel / 仅发布脚本且不变更 KPanel：
- 阻断或移除的依赖范围（无则写“不适用”）：
...
- 直接/基座行动项、传递依赖归属信号及首次完整检测后的启动/决策/处置期限：
...
- 本地资源回收（按 `docs/project-management.md` 13.1）：已回收路径及实际净释放字节 / 跳过原因 / 保留的证据与恢复位置；未执行时如实说明。
