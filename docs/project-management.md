...
规范的有效版本以已批准的精确 Git 基线为准；未提交草稿、未集成候选和历史提案不自动成为主线规范。
管理工作树落后时，使用对应基线的专用工作树或 `git show <exact-ref>:<path>` 读取规范，不把旧检出误当最新。
文档索引区分当前规范与历史证据；不得把旧提案的“待复核”状态当作当前任务队列，也不批量改写历史验收。
界面与视觉语言的产品入口为 [`docs/ui-visual-language.md`](ui-visual-language.md)；组件或业务设计文档只记录
...
禁止路径、基线提交、验证等级、依赖和推送/发布权限。用户明确要求修改、开发或修复 KPanel，即授权
任务在专用分支形成非空、聚焦的本地任务候选提交；这是写任务的完成条件，不授权 SSH 推送、更新
`main`、Tag、Release、镜像或生产。跨智能体交接不得依赖未提交文件或聊天记忆。
...
| 开发任务 | 在专用 worktree 内实现和测试，形成聚焦的本地任务候选提交 | 修改 `main`、未经授权推送、推标签、部署、操作其他任务工作树 |
...
主要实现与最终验证/发布的独立性按第 8 节判断；验证和发布可以由同一个接手任务承担，不要求第三个常驻复核任务。
进入发布通道后的职责统一按第 10.1 节执行，通用开发任务复用规则不得覆盖发布接管。
...
- “待验证”必须已有聚焦差异和任务级自验；包含仓库修改时必须已经形成本地任务候选提交并保持
  worktree clean。仍是 dirty 差异时状态保持“执行中”或“阻塞”，不得写成完成、交接或待集成。
...
本地功能预览：不适用及依据 / draft / 候选提交后 acceptance；mock / integration / isolated-real-host；入口、旅程、证据目录和停止方式：
权限：修改与本地任务候选提交 / SSH 推送 / 更新 main / tag / Release / 部署：
交付物：差异 / 提交 / 报告 / 产物：
```
...
4. 写任务的本地任务候选提交随修改授权生效；是否允许 SSH 推送、合并、打标签或部署仍须分别明确，
   未明确授权一律视为不允许；
...
5. L0-L3、受影响用户旅程、自动/实机/浏览器证据和权限边界明确；远程目标已通过环境用途检查，
   ...
   有可见界面变化时已按 `docs/local-feature-preview-standard.md` 选择预览模式和等级，并计划在候选提交后
   提供绑定精确提交的 acceptance 预览；不能提供本地预览时写明真实能力边界；
...
5. 上述治理变更通过本地门禁后仍只形成治理候选；获得推送和主线授权时，先推送专用非 `main` 分支并
   ...
   失败当作候选测试。候选分支在主线 CI 成功前不得删除；成功后按第 10.2 节归档。
...
开发任务只有形成通过完成检查的本地任务候选提交后，才能向协调中心回传同一结构的完成交付包；
交付包进入发布通道后由发布任务保管和补齐，不再向原任务返派：
...
5. 只要实际修改仓库，就已在专用非 `main` 分支形成至少一个相对精确基线非空的聚焦候选提交，
   worktree clean，并通过
...
9. 涉及用户可见页面、交互、文案、布局或状态反馈且能在本地安全复现时，已在候选提交后提供绑定
   当前精确提交的 acceptance 预览和统一预览卡；候选变化后已重做受影响预览。预览保持 ready 供用户
   验收时，已明确 manifest、停止命令和当前责任人；用户验收结束或完成移交后再停止。纯后端、文档、
   测试/构建基础设施或只能在登记隔离真机验证的任务已记录不适用依据，不制造无意义预览。
...
只读分析、经核对确认无需修改、用户取消或真实阻塞可以不产生提交，但必须分别标记为“只读完成”、
“无需修改关闭”、“已取消”或“阻塞”，不能宣布仓库写任务完成。禁止用空提交满足本节。
...
1. 原智能体停止写入；正常完成时必须形成任务候选提交并保持 clean，只有取消或阻塞才能明确记录未提交
   文件，不得把模糊工作区作为完成交付给下一方；
...
候选移交发布任务即结束原开发任务在该候选内的写入责任。旧任务只作为已有提交、文件和证据的查询入口，
不再承担该候选的修复、工具准备、封包或验收；后续由发布任务按第 10.1 节自行完成。

涉及 L2 高风险变更时，优先由不同智能体独立复核；L3 发布必须由与主要实现任务不同的验证/发布任务
检查提交清单、门禁和回滚点。该接手发布任务可以同时完成最终验证；不为重复审批另设复核任务。
若另一种 AI 暂不可用，使用与主要实现分离的干净会话完成并记录这一限制。
...
3. 集成任务从最新 `origin/main` 建立干净 worktree，逐项重放批准提交；发生冲突先停止并记录，确认基线和所有权后由集成任务在专属工作树处理、记录并重验，不向已交付的原开发任务返派，不在共享主工作树试错。
...
- 候选和已交付任务分支按第 10.2 节归档；稳定版公开后的自动步骤仅负责远端引用收尾，
  不代表公开镜像 E2E、生产部署、验收记录或本地资源回收已完成。预览版保留同一序列候选。
...
1. **接管点**：精确候选范围和标准交付包已移交，原写入者释放所有权，唯一发布任务核对后声明接管。
   接管不以原作者继续在线为前提；未完成的验收和已声明草稿须如实列明，由接手者核实，不能改记为通过。
2. **执行责任**：发布任务自行完成差异审查、验收设计、工具/环境准备、失败诊断、修复、封包、CI、产物、
   备份、标准部署和回滚。源码修复使用自己拥有的专属修复分支；外部验收工具也由其唯一写入，不因位于仓库外而返派。
3. **停止返派**：不得重新唤醒已交付的开发或验证任务处理本候选，不恢复旧 writer，不新增替代复核任务。
   已有原件可只读复用；遗留工作树只有在原作者已停止、内容与权限明确移交后才能按第 8 节接手，不能覆盖未知改动。
4. **沟通边界**：协调中心不定时催问发布任务、不对普通执行细节逐项审批。发布任务可按作业协议有界读取
   自己的后台终态并保留 watchdog；后台运行不要求额外监控会话。只有超出用户授权、未知所有权/冲突或无法
   在既定范围内安全继续的事项才升级处理；正常进度和可自行修复的问题不产生反复交接。
5. **质量与权限**：接管不减少 L3、候选/主线 CI、真实业务证据、产物核对、备份和回滚要求。改变候选或验收
   参数时重新验证受影响部分，保留首轮失败。新功能进入下一版本；永久治理仍须非作者复核和同 SHA 候选 CI。
6. **结束条件**：发布任务完成既定交付或明确终止后一次性给出结果。协调中心读取实际产物/生产证据后再复盘
   并安排下一轮；单个回合结束、任务暂时空闲或等待超时不能当作发布完成。治理候选的结束以其集成/CI 状态为准，
   不制造产品版本或生产部署。交付必须包含第 10.2 节归档结果；归档失败时分别报告已发布事实与待收尾项，
   不因清理失败重复打版本、重发镜像或重做生产部署。

### 10.2 分支归档与下一轮候选筛选

本节是归档生命周期的唯一权威入口。归档表示保存精确提交和处置依据，并将已结束分支移出活跃队列；
不等于删除工作树、已上线、已合并或已验证。默认使用 `archive/<原分支全名>`，归档引用只读、不得覆盖；
名称冲突且 SHA 不同时停止该项；自动入口对“候选与归档同时存在”一律停止，即使 SHA 相同也不直接删除，
避免 Git 将同值归档视为 no-op 而跳过 lease。恢复开发应从批准基线建立新任务分支，按证据重放所需提交，不直接续写归档。

| 对象 | 归档触发及必要证据 | 保留条件 |
| --- | --- | --- |
| 稳定发布 `release/vX.Y.Z-candidate` | 候选/主线 CI、稳定 Release 和公开产物发布成功，候选 tip 是正式 tag 的祖先或同一提交；自动保存精确 tip 并移除原远端引用 | 分叉、查询失败、归档冲突或远端 tip 改变时停止；标签后新增的验收提交不随旧标签强删 |
| 预览发布候选 | 单次 RC 发布不归档；同序列稳定版完成上述步骤后归档 | RC 期间保留唯一候选；废弃序列须有明确处置授权与恢复依据 |
| 已纳入版本的功能/修复/组装分支 | 发布结束收尾时核对精确提交、纳入版本及原作者已释放所有权，再保存并移出活跃区 | 不能只按名称、时间或 `--merged` 判定；重放/squash 需精确映射、差异和验收证据 |
| 纯治理及验收记录候选 | 同一精确 SHA 的候选 Linux CI、主线快进与主线 CI 成功后归档 | 主线 CI 未结束前保留；不等待或制造产品版本 |
| 未合并、被替代或废弃分支 | 有明确负责人处置决定、保留精确 tip 和恢复位置后归档，明确标为未上线/被替代/废弃 | 独有提交、未知归属或下一轮工作不能默认为已发布 |

**执行与权限：**

1. 发布任务在发布前固定本次候选与来源分支清单；发布结束后负责该清单的收尾。协调中心在下一轮选取前
   检查遗留项，不召回旧任务、不全量扫描历史归档来猜候选。已经授权的精确分支归档可直接完成；普通
   开发授权不包含远端写入，范围外分支不得搭车清理。分支归档授权不等于生产或工作树删除授权。
2. 稳定候选唯一自动入口为 `node scripts/archive-release-candidate.mjs --tag <stable-tag>
   --release-sha <exact-release-sha>`；默认只核验（会 fetch Git 对象，不写远端），仅明确授权的发布
   workflow 在公开产物后加 `--apply`。脚本核对远端 tag、祖先关系及精确 tip，用原子 push 和两端
   expected-SHA lease 同时保存归档与删除活跃引用，再读取远端确认。执行前输出原 tip 与 tag 的恢复计划。
   网络/鉴权失败不是“已经不存在”；推送超时或写后核验失败表示状态待核对，不能断言引用未变化。
   Git 可能省略并发创建的同值归档更新，lease 不保护 no-op 或后续违规改写归档；因此保留不可变 tag
   和原 tip，远端复核失败时先重读引用，必要时从正式 tag 可达的原 tip 恢复到新分支，禁止覆盖冲突归档。
   自动步骤不接收任意任务分支；其他已授权分支按同样的精确引用、先保留、核验、受 lease 保护的移除
   顺序由当前责任人执行，不能直接使用无 expected-SHA 的批量删除。
3. 重试已完成操作只接受可复核的同 SHA 归档与正式 tag；原分支和归档都不存在时停止并查历史证据。
   历史已删除分支若能由不可变 tag 或已验证 bundle 精确恢复，可以记录为“历史已归档”，不为统一命名
   重建已结束候选。已有 `archive/` 分支按精确 SHA 保留，归档本身不能证明内容已上线。
4. 发布收尾在现有验收记录中列出：原分支、原 tip、处置分类、纳入版本/主线或替代依据、归档 ref/SHA
   （历史项可为 tag/bundle）、远端复核结果、保留理由和责任人。生产已授权时，postdeploy、验收和归档
   处置全部完成才算完整上线交付；仅产物授权时明确“生产未部署”。归档受阻必须给出恢复步骤和下次
   复核触发条件，不能写成已完成，也不能抹去已经发生的发布或部署事实。
5. 本地旧分支、upstream 和 worktree 单独核对：只有归档验证、所有权释放、clean 且无独有提交时，才在
   授权范围内回收并解除旧 upstream，防止普通 push 重建候选。含未提交/未跟踪文件或未知所有权的工作树
   原样保留，列为“本地待处置”，不再算待发布候选；资源回收仍按第 13.1 节，不强删、不自动 stash。
6. 恢复时先 fetch 并验证归档 SHA，在新任务 worktree 从该精确提交读取或重放；不得覆盖现有分支。
   历史 bundle 必须先 `git bundle verify <exact-path>`，并在临时独立仓库验证目标提交可读后才作为恢复依据。

**下一轮的最小只读盘点（PowerShell）：**

```powershell
git fetch origin --prune
git for-each-ref --format='%(refname:short) %(objectname)' refs/remotes/origin |
  Where-Object { $_ -notmatch '^origin/(archive/|HEAD |main )' -and $_ -notmatch '^origin ' }
git worktree list --porcelain
```

只对非归档分支逐项记录精确 tip、与批准基线的差集及“本轮/后续/待处置”决定；本地未推送候选另行
盘点，不因远端缺失而丢失。`archive/` 仅在明确恢复或追溯时查询；已经纳入 `main` 也不等于已经上线。
上述归档清单复用现有交付包/验收记录，不创建平行台账；机器测试只证明引用处理安全，不代替所有权、
公开产物、生产和重放等价性的证据。
...
| 创建聚焦本地任务候选提交 | 用户明确要求修改、开发或修复 KPanel 时允许，且为写任务完成条件 |
| 推送功能/候选分支 | 需要明确授权 |
...
- 每个开发任务：Definition of Ready 后启动，完成后只回传一个标准交付包；中间只报告关键风险、
  范围变化或阻塞，不重复发送无状态进展。
- 每个发布窗口：按第 10.1 节由发布任务独立闭环；协调中心等待完成交付，不创建周期巡检或把修复派回旧任务。
- 每次版本：先形成精确发布范围，再冻结；远程 L3 统一由 `scripts/run-release-l3.mjs` 生成自包含
  执行包并调用 `scripts/run-release-gate.sh`，随后进入候选 CI、主线、标签、隔离验收和授权生产部署；
...
本节管理开发机上的 KPanel 工作树、依赖、临时产物和本地证据，不管理生产磁盘，也不授权清理同一
父目录下其他项目。清理采用任务生命周期触发，不创建周期监工，不为释放空间召回旧任务。

**触发与责任：**

- 创建新 worktree、安装依赖或生成大型归档前，当前责任任务查看已有同用途资源及磁盘余量；优先使用
  自己已持有的工作树和有效缓存。规范/文档任务不为通过治理检查安装整套前端依赖。
- 开发交付后，原作者停止自己已不需要的预览/测试进程，移交所需原件与路径；清理自己明确可再生且
  未被接手者依赖的临时文件。供发布复核的工作树在接管完成前保留，交付不等于允许立即删除。
- 发布结束后，由发布任务处理自己持有的候选资源并记录回收结果；协调中心收到结束交付后、安排下一轮前，
  统一盘点已释放所有权的历史资源。发布期间不清理其路径、不催问进度、不另设清理会话。
- 本地工作区父目录逻辑用量达到 `30 GiB`，或所在卷可用空间不足 `20 GiB` 时，触发下一次上述边界的
  优先回收；这是初始预警线，不是删除授权、产品门禁或必须降到指定数字的指标。创建大型资源前空间不足时，
  当前任务先减小临时副本或回收已确认可删除项，不能让磁盘耗尽后才处理。

**按以下顺序回收：**

| 内容 | 可回收条件 | 必须保留 |
| --- | --- | --- |
| `node_modules`、构建输出、临时解包/源码副本、浏览器临时 profile | 所属任务结束、无进程使用、由 lockfile/精确源码可再生；确认不含唯一证据、用户数据或秘密 | 活跃开发、预览、后台验收和发布使用的目录 |
| 已交付 worktree | 所有权已释放；clean；提交可从批准 `main`、不可变 tag 或已验证归档恢复；无未合并独有提交；已检查忽略文件和嵌套仓库 | 管理工作树、当前候选、未提交修改、未知归属、用户保留的预览与回滚工作树 |
| 重复安装包、镜像导出、bundle、完整源码归档 | 已核对精确 SHA/摘要及可恢复位置，既有副本足够恢复；无未结束的验收/排错依赖 | 当前与上一稳定版本仍需本地恢复的唯一副本；不能以“已上传”代替恢复验证 |
| 历史原始 trace、截图、视频和大型日志 | 超过 `30 天`、不属于当前/上一稳定版本、无未解决缺陷或观察窗口，且所需原件已有经过摘要核对的可访问归档 | Git 内发布验收、首轮失败/根因证据、回滚记录及唯一原件；年龄本身不是删除依据 |

**执行与验收：**

1. 默认先只读盘点，记录每个目标的规范化绝对路径、所属项目/任务、大小口径、Git/进程状态、保留理由
   或可回收依据。逻辑文件大小可能重复计算硬链接，不能直接宣称是可释放空间；符号链接、junction 和
   其他 reparse point 不递归跟随。目录名称带 `tmp`、任务空闲或分支名称陈旧均不构成删除依据。
2. 删除前逐项复核路径仍在已批准工作区内且不是根目录、管理树或受保护路径；祖先/目标存在链接、
   所有权不明、仍有进程、Git 状态变化或恢复检查失败时跳过并保留，不自动 stash、提交未知修改或强删。
   活跃任务的文件不能因磁盘预警被另一个任务接管清理。
3. 在任务已有清理授权范围内处理精确对象；重要归档、唯一副本或未明确授权的跨任务删除须先取得目标授权。
   linked worktree 使用 `git worktree remove <exact-path>`，失败即停止该项，禁止 `--force`；分支按第 10 节
   的可恢复规则另行处理，不能顺带删除未合并分支。普通目录在 Windows 使用同一 PowerShell 会话的
   `Remove-Item -LiteralPath`，不拼接 `cmd /c`、不通配批量删除、不运行 `git clean -fdx` 或全局 prune。
4. 复测所在卷空闲空间和保留工作树状态，在现有交付包记录删除项、跳过项、实际净释放字节、剩余用量及
   恢复位置。空间未下降或删除失败如实报告，不把预计大小当完成结果；不为每次回收再制作整仓归档。

本节是明确的执行责任和保留策略；现有治理检查只验证入口与关键约束，没有后台删除器，也不自动证明
某个本地目录可删。仓库外旧的一次性清理脚本须重新审查，不得复用其中的自动 stash、主分支改写或强制移除。
...
3. **准入**：由独立复核者读取原始证据并完成 Definition of Ready，排除指标投机、规则自我弱化和
   与产品核心思想冲突的方案。用户明确授权本地实施时可先形成可复核候选；正式准入和永久采纳仍须
   非作者复核，不把尚未完成的复核写成通过，也不为此恢复旧任务或打断正在执行的发布；
...
该循环不能自动降低安全/质量门禁，也不能自行启动写任务。用户授权实施后必须形成本地任务候选提交，
但不能自动 SSH 推送、更新主线、发布或操作生产。可复用执行步骤见
...
1. 定时 CI 和 `make dependency-report` 只读取上游稳定来源，输出候选、检测源完整性、行动项/传递依赖
   归属信号和采用时限，不写代码；
2. 协调中心按安全可达性、EOL、补丁/次/主版本、工具链/基础组件和产品收益建立聚焦任务。完整检测首次
   确认后，Patch 最晚 7 天启动/14 天决策/30 天完成处置，Minor 为 14/30/60 天，Major 或 digest/pin/
   revision 等非 SemVer 基座变化为 30/90/90 天；基座 Patch/Minor 使用较短期限但仍保持 L2/L3 验收，
   安全事项服从漏洞响应更严格时限；
3. 完成处置是采用、以证据拒绝或建立有期限例外。暂缓候选按 `dependency-policy.json` 建立有负责人、
   复核日期、缓解、退出条件和回滚点的例外；重复报告不能重置期限；
4. 直接依赖和基座作为行动项；传递依赖跨父范围的 `latest` 由拥有它的直接依赖、锁文件刷新或可达安全
   路径处置，不逐项机械强升，也不得从报告中隐藏；
5. 升级在独立 worktree 实施，补丁按影响 L1/L2，次版本至少 L2，主版本、工具链、基础镜像、Action、
   扫描器和受管脚本执行 L2/L3；
6. 只有门禁和授权满足后才允许任务分支、快进主线、发布或生产进入各自状态；采用后观察失败、资源、
