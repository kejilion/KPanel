...
- 提案状态：待复核（实现候选已形成，待独立复核、同 SHA Linux CI 与主线采纳）
- 提案日期：2026-09-21
...
- 提交、推送、发布和生产权限：用户授权完善本地 WSL 灾备通道并形成本地候选；不包含推送、主线、版本发布或生产写入
...
- 未确认：当前机器没有可信旧 Runner 归档，因此本提案不能把本轮 rc.7 L3 写成已通过，也不把网络不可达解释为产品失败。
...
- 定向测试：环境策略与 L3 orchestrator 回归；Shell 语法；真实 WSL 的错误 Runner 关闭失败。
- `make verify-change`：候选提交后执行；结果在本提案最终复核时填写。
- 隔离真机或浏览器：浏览器不适用；真实 WSL 只验证执行通道和失败边界。
- 公开产物与生产：不执行，不适用。
- 未验证项：可信固定 Runner 归档尚不存在，完整 WSL L3 成功必须在归档恢复后对精确候选重跑。

## 独立复核

- 复核人 / 智能体：待安排
- 复核提供商 / 实现提供商：待复核 / Codex
- 是否独立读取原始证据：待复核
- 假设与方案评审结论：待复核
...
- 决策：继续试行；待独立复核、同 SHA Linux CI 与主线授权。
- 规范验收结论：待复核
- 后续事项：从已验收环境导出精确 Runner 归档并保存摘要后，执行首轮完整 WSL L3 演练。
