...
Compose 与 `.env` 并再次 `up`，但仅限文件仍处于本任务预期状态。Agent 在校验命令返回后、每次
替换或恢复文件前以及运行态补偿前，复核完整配置集合、`.env` 存在性、内容、文件和目录身份。
发现外部编辑时保留当前文件，停止后续写入与补偿命令并提示检查；配置恢复或运行态恢复失败时也
保留明确的“需要人工处理”状态。该复核保护长命令窗口和失败恢复，不对未参与协调的 SSH/编辑器
提供跨检查与 rename 的原子 CAS 保证。
