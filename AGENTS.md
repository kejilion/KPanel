...
- 使用等待/消息能力只是本地协调优化；协调中心必须从精确差异、提交和验证证据独立验收，不能只转发
  子任务自述。
- 管理目录落后时读取任务精确基线中的规范，不将旧检出或未集成草稿作为当前共享规则。
- 新建工作树前与任务/发布结束后，按 `docs/project-management.md` 13.1 盘点并回收本地磁盘；不清理活跃、未知归属或未提交内容。
...
仓库写任务在结束前必须形成相对精确基线非空的本地任务候选提交，并运行
`node scripts/check-collaboration-state.mjs --role writer --base-ref <exact-base> --require-candidate`；dirty 差异
只能是执行中、取消或阻塞，不能宣布完成。只读或确认无需修改时不制造空提交。
候选提交涉及本地可安全复现的用户可见页面、交互、文案、布局或状态反馈时，随后必须按
`docs/local-feature-preview-standard.md` 提供绑定该提交的 acceptance 预览；候选改变后重新预览。纯后端、
文档、测试/构建基础设施或只能在登记隔离真机验证时写明不适用依据。

达到 Definition of Done 后回传 `docs/project-management.md` 的标准交付包，至少包含：精确 Git 状态、
修改文件、已验证事实、质量维度和证据层级、命令与结果、未验证风险、依赖/冲突、回滚和权限状态。

- 用户要求修改、开发或修复 KPanel 即授权在专用分支形成聚焦的本地任务候选提交；不自动授权 SSH
  推送、更新 `main`、tag、Release 或部署。
