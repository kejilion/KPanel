...
- 约束臂 6 条；自由臂在知晓这 6 条的前提下新增 7 条，并集 13 条，重合 46%；唯一 HIGH（批次 header 与移动端 host 选择条重叠锁死
...
| 2026-09-19 | 1.12.6（`7a571b78`，GitHub 正式版 / npm latest） | PASS / PASS / PASS（评审者事先知道 F1/F2，非盲测；以确定性证据补强：1.12.5 与 1.12.6 对 canary 的 `delegate rule` 输出与 `delegate preview` 输出 sha256 逐字节一致，评审输入不变）/ PASS | 采用，pin 升至 1.12.6 | 本记录；上游变更仅限查看器、IDEA 插件、更新器窗口与测试隔离，委托路径（圈选、规则、默认排除）无差异；`.vue` 仍为通用 `default` 规则 |
