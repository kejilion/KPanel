...
describe('DesktopView', () => {
  ...
  it('supports arrow-key menu navigation and restores focus on Escape', async () => {
    ...
    expect(menu.attributes('data-context-menu-focus')).toBe('pointer')
    expect(items[0]?.attributes('data-context-menu-active')).toBeDefined()
    expect(items[0]?.attributes('aria-selected')).toBeUndefined()

    await items[0]!.trigger('keydown', { key: 'ArrowDown' })
    expect(menu.attributes('data-context-menu-focus')).toBe('keyboard')
    expect(document.activeElement).toBe(items[1]?.element)
    expect(items[0]?.attributes('data-context-menu-active')).toBeUndefined()
    expect(items[1]?.attributes('data-context-menu-active')).toBeDefined()

    await menu.trigger('pointermove')
    expect(menu.attributes('data-context-menu-focus')).toBe('pointer')
    expect(document.activeElement).toBe(items[1]?.element)
