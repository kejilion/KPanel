...
describe('DesktopView icon layout interaction', () => {
  ...
  it.each(['workspace', 'entries'])('keeps restored placement stable with a delayed %s response', async slow => {
    ...
    loadEntries.mockReturnValue(entriesRequest.promise)
    const saved = workspace({ groups: [{ id: 'a'.repeat(32), name: 'Saved', columns: 4, rows: 0, collapsed: false, members: ['nav:/overview', 'app:example'] }] })
    const app: DesktopEntry = { key: 'app:example', id: 'example', kind: 'app', name: 'Example', launch: 'external', url: 'https://example.com' }
    const catalog: DesktopEntries = { apps: [app], sites: [], visible: [app], loadedAt: Date.now() }
    ...
    await flushPromises()
    expect(grid.classes()).not.toContain('desktop__icons--initializing')
    expect(grid.attributes('inert')).toBeUndefined()
    ...
    // Returning from classic mode measures the new surface before exposing cached groups.
    const remountRequest = deferred<DesktopWorkspace>()
    loadWorkspace.mockReturnValue(remountRequest.promise)
    const remounted = mount(DesktopView, { attachTo: document.body })
    await flushPromises()
    expect(remounted.get('.desktop__icons').classes()).toContain('desktop__icons--initializing')
    remountRequest.resolve(saved)
    await flushPromises()
    expect(remounted.get('.desktop__icons').classes()).not.toContain('desktop__icons--initializing')
    expect(remounted.get('[data-icon-key="nav:/overview"]').attributes('data-group-member')).toBe('a'.repeat(32))
    remounted.unmount()
  })

  it('releases the initial loading surface when requests fail', async () => {
    loadWorkspace.mockRejectedValue(new Error('offline'))
    loadEntries.mockRejectedValue(new Error('offline'))
    const wrapper = mount(DesktopView, { attachTo: document.body })
    await flushPromises()
    expect(wrapper.get('.desktop__icons').classes()).not.toContain('desktop__icons--initializing')
    expect(wrapper.get('.desktop__icons').attributes('inert')).toBeUndefined()
    expect(wrapper.find('[data-icon-key="nav:/overview"]').exists()).toBe(true)
    wrapper.unmount()
  })
