...
watch(() => props.iconURL, () => { failed.value = false; loaded.value = false })
</script>

<template>
  <span class="desktop-group-preview-icon" :title="label" aria-hidden="true">
    <img v-if="iconURL && !failed" :src="iconURL" :style="{ opacity: loaded ? 1 : 0 }" alt="" width="26" height="26"
      draggable="false" decoding="async" referrerpolicy="no-referrer" @load="loaded = true" @error="failed = true" />
    <template v-if="!loaded || failed">
      <component v-if="icon" :is="icon" :size="18" :stroke-width="1.7" />
...
.desktop-group-preview-icon > * { grid-area: 1 / 1; }
</style>
