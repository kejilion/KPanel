...
let disposed = false
const config = computed(() => JSON.stringify({ mcpServers: { kpanel: { url: settings.value?.endpoint, headers: { Authorization: `Bearer ${token.value}` } } } }, null, 2))
const canCreate = computed(() => settings.value?.access.enabled && settings.value.access.available && settings.value.transportReady && !busy.value && !token.value && name.value.trim().length > 0 && selected.value.length > 0 && settings.value.access.clients.length < settings.value.maxClients)
...
async function toggle() {
  ...
  try {
    ...
    notice.value = value.access.enabled ? 'MCP 已启用，请创建客户端。' : 'MCP 已关闭，所有客户端连接已停止。'
...
async function create() {
  ...
  busy.value = true; error.value = ''; notice.value = ''
  try {
    const result = await mcpAccess.create({ name: name.value.trim(), hostIds: [...selected.value], expiresInDays: days.value, expectedResourceVersion: settings.value.access.resourceVersion })
    if (disposed) return
...
async function test() {
  busy.value = true; error.value = ''; notice.value = ''
  try { await mcpAccess.test(token.value); if (!disposed) notice.value = '连接成功，巡检权限已验证。' }
...
<template>
  <section id="mcp-access" class="settings-section panel-card mcp-access" aria-labelledby="mcp-title">
    <header class="settings-section__header">
      ...
      <div><h2 id="mcp-title">MCP 接入</h2><p>让外部 AI 客户端查看本机资源与集群状态，无需配置面板内置 AI。</p></div>
    </header>
    <div v-if="loading" role="status">正在加载 MCP 接入…</div>
    <div v-else-if="settings" class="mcp-content">
      <div class="mcp-toolbar">
        <div><strong>{{ settings.access.enabled ? '已启用' : '未启用' }}</strong><p class="mcp-note">巡检权限：可读取所选主机摘要，以及本机容器、网站和已安装应用；不包含文件、日志、终端或写操作。</p></div>
      ...
      <form v-if="settings.access.enabled" class="mcp-form" @submit.prevent="create">
        ...
        <fieldset :disabled="busy || !!token"><legend>允许查看的主机</legend><div class="mcp-hosts"><label v-for="host in hosts" :key="host.id"><input v-model="selected" type="checkbox" :value="host.id" /><span>{{ host.name }} <small>{{ host.isLocal ? '本机资源与摘要' : '集群摘要' }}</small></span></label></div></fieldset>
        ...
        <button class="button button--primary" type="submit" :disabled="!canCreate">创建巡检客户端</button>
      </form>
      ...
      <div class="mcp-history">
        ...
        <ul v-else>
          <li v-for="client in settings.access.clients" :key="client.id">
            <div><strong>{{ client.name }}</strong><p>{{ client.hosts.map(h => hostLabel(h.id)).join('、') }}</p><small>{{ Date.parse(client.expiresAt) <= Date.now() ? '已过期' : '到期时间' }} · {{ formatDateTime(client.expiresAt) }}</small></div>
...
.mcp-form input:not([type=checkbox]), .mcp-form select, .mcp-credential textarea { min-height: 40px; padding: 8px 12px; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--surface); color: var(--text); font-size: 14px; width: 100%; min-width: 0; box-sizing: border-box; }
