...
describe('FilesView remote download', () => {
  ...
  it('presents the feature as a regular remote download in every locale', () => {
    ...
    expect(zhCN).toContain("'files.remoteDownload.note': '支持公开 HTTP/HTTPS 地址，单个文件最多 512 MiB。Panel 重启时任务会标记为中断且不自动重试；完整 URL 不会保存。'")
    ...
    expect(zhTW).toContain('"files.remoteDownload.note": "支援公開 HTTP/HTTPS 網址，單一檔案最多 512 MiB。Panel 重新啟動時任務會標記為中斷且不自動重試；完整 URL 不會儲存。"')
    ...
    expect(enUS).toContain("'files.remoteDownload.note': 'Supports public HTTP/HTTPS URLs up to 512 MiB per file. A Panel restart marks active downloads interrupted with no automatic retry, and the full URL is never saved.'")
