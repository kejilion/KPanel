...
const runState=computed(()=>{i18n.locale.value;if(!runActive.value)return{label:i18n.t('ai.run.ready'),tone:'idle'};if(currentRun.value?.status==='pending_approval')return{label:i18n.t('ai.run.pendingApproval'),tone:'approval'};if(!connected.value)return{label:i18n.t('ai.run.reconnecting'),tone:'offline'};return{label:i18n.t('ai.run.generating'),tone:'online'}})
...
const grouped=computed(()=>{i18n.locale.value;const result:{label:string;items:AISession[]}[]=[];const now=new Date();for(const item of sessions.value.filter(item=>item.title.toLowerCase().includes(search.value.toLowerCase()))){const date=new Date(item.lastMessageAt);const days=Math.floor((new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime()-new Date(date.getFullYear(),date.getMonth(),date.getDate()).getTime())/86400000);const label=showArchived.value?i18n.t('ai.session.archived'):item.pinned?i18n.t('ai.session.pinned'):days===0?i18n.t('ai.session.today'):days===1?i18n.t('ai.session.yesterday'):i18n.t('ai.session.earlier');let group=result.find(group=>group.label===label);if(!group){group={label,items:[]};result.push(group)}group.items.push(item)}return result})
...
function toolStatusLabel(call:AIToolCall){i18n.locale.value;if(call.status==='pending_approval')return i18n.t('ai.tool.pendingApproval');if(call.status==='running')return i18n.t('ai.tool.running');if(call.status==='completed')return i18n.t('ai.tool.completed');if(call.status==='failed')return i18n.t('ai.tool.failed');if(call.status==='rejected')return i18n.t('ai.tool.rejected');return call.status}
...
async function loadMessages(){closeStream();messages.value=[];messageCursor.value='';toolCalls.value=[];currentRun.value=undefined;resetStream();attachments.value=[];followOutput.value=true;if(!activeId.value)return;try{await refreshConversationMessages();scrollBottom(true);const session=active.value;if(session?.running&&session.activeRunId)openStream(session.activeRunId);else if(session?.lastRunId&&['interrupted','failed'].includes(session.lastRunStatus||''))currentRun.value=await aiApi.runs.get(session.lastRunId)}catch(reason){error.value=localizeError(reason,'ai.error.conversationLoad')}}
async function loadOlder(){if(!activeId.value||!messageCursor.value||loadingOlder.value)return;loadingOlder.value=true;try{const page=await aiApi.sessions.messages(activeId.value,messageCursor.value);messages.value=[...conversationMessages(page.items),...messages.value];messageCursor.value=page.nextCursor||''}catch(reason){error.value=localizeError(reason,'ai.error.olderMessages')}finally{loadingOlder.value=false}}
async function openNewSession(){const model=defaultModel.value;const provider=providers.value.find(item=>item.id===model?.providerId);if(!provider||!model){settingsOpen.value=true;return}if(creatingSession.value)return;creatingSession.value=true;error.value='';try{const item=await aiApi.sessions.create(provider.id,model.id);showArchived.value=false;sessions.value.unshift(item);await router.push(`/ai/s/${item.id}`);sessionDrawer.value=false;await nextTick();composer.value?.focus()}catch(reason){error.value=localizeError(reason,'ai.error.createSession')}finally{creatingSession.value=false}}
async function send(){const content=input.value.trim();const selected=attachments.value;if((!content&&!selected.length)||!active.value||sending.value)return;sending.value=true;error.value='';input.value='';attachments.value=[];void nextTick(()=>{if(composer.value)composer.value.style.height='auto'});messages.value.push({id:`local-${Date.now()}`,sessionId:active.value.id,role:'user',content,attachments:selected.map(({name,mimeType,size,kind,previewUrl})=>({name,mimeType,size,kind,previewUrl})),createdAt:new Date().toISOString()});scrollBottom(true);try{const result=await aiApi.sessions.send(active.value.id,content,selected);openStream(result.runId);void refreshSessions()}catch(reason){input.value=content;error.value=localizeError(reason,'ai.error.send');await loadMessages();attachments.value=selected}finally{sending.value=false}}
...
async function decide(approve:boolean){const call=pendingApproval.value;if(!call||!currentRun.value)return;try{await aiApi.runs.decision(currentRun.value.id,call.id,approve);call.status=approve?'running':'rejected';openStream(currentRun.value.id)}catch(reason){error.value=localizeError(reason,'ai.error.approvalSubmit')}}
async function cancelRun(){if(!currentRun.value||cancelling.value)return;cancelling.value=true;error.value='';try{await aiApi.runs.cancel(currentRun.value.id);closeStream();await refreshSessions();await loadMessages()}catch(reason){error.value=localizeError(reason,'ai.error.stopRun')}finally{cancelling.value=false}}
async function composerAction(){if(composerStopsRun.value){await cancelRun();return}await send()}
async function retryRun(){if(!currentRun.value)return;try{const result=await aiApi.runs.retry(currentRun.value.id);openStream(result.runId);await refreshSessions()}catch(reason){error.value=localizeError(reason,'ai.error.retry')}}
...
async function updateModel(modelId:string){const model=models.value.find(item=>item.id===modelId);const provider=providers.value.find(item=>item.id===model?.providerId);if(!active.value||!model||!provider)return;error.value='';try{const updated=await aiApi.sessions.update(active.value.id,{providerId:provider.id,modelId:model.id});sessions.value=sessions.value.map(item=>item.id===updated.id?updated:item)}catch(reason){error.value=localizeError(reason,'ai.error.modelSwitch')}}
async function updateApprovalMode(value:string){const approvalMode=value as AIApprovalMode;if(!active.value)return;error.value='';try{const updated=await aiApi.sessions.update(active.value.id,{approvalMode});sessions.value=sessions.value.map(item=>item.id===updated.id?updated:item)}catch(reason){error.value=localizeError(reason,'ai.error.approvalModeSwitch')}}
async function updateThinkingLevel(value:string){const thinkingLevel=value as AIThinkingLevel;if(!active.value)return;error.value='';try{const updated=await aiApi.sessions.update(active.value.id,{thinkingLevel});sessions.value=sessions.value.map(item=>item.id===updated.id?updated:item)}catch(reason){error.value=localizeError(reason,'ai.error.thinkingLevelSwitch')}}
async function rename(item?:AISession){const target=item||active.value;if(!target)return;const title=prompt(i18n.t('ai.sessionName'),target.title)?.trim();if(!title)return;try{const updated=await aiApi.sessions.update(target.id,{title});sessions.value=sessions.value.map(value=>value.id===updated.id?updated:value)}catch(reason){error.value=localizeError(reason,'ai.error.renameSession')}}
async function togglePin(item:AISession){try{const updated=await aiApi.sessions.update(item.id,{pinned:!item.pinned});sessions.value=sessions.value.map(value=>value.id===updated.id?updated:value)}catch(reason){error.value=localizeError(reason,'ai.error.pinSession')}}
...
async function archive(item:AISession){try{await aiApi.sessions.update(item.id,{archived:true});await refreshAfterSessionLeaves(item)}catch(reason){error.value=localizeError(reason,'ai.error.archiveSession')}}
      ...
      <div class="ai-session-groups">
        <section v-for="group in grouped" :key="group.label"><h3>{{group.label}}</h3>
          <div v-for="item in group.items" :key="item.id" class="ai-session-item" :class="{active:item.id===activeId}">
            <RouterLink :to="`/ai/s/${item.id}`" @click="sessionDrawer=false"><strong>{{item.title}}</strong><small>{{item.modelName}} · {{sessionTime(item.lastMessageAt)}}</small></RouterLink>
