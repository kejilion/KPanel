...
describe('ClusterView compact summary layout', () => {
  it('keeps summary metrics and actions on one decorated row', () => {
    ...
    expect(source).toMatch(/\.cluster-metric-link:hover\s*\{[^}]*background:\s*var\(--interaction-hover-surface\);/)
    expect(source).toMatch(/\.cluster-metric-link:focus-visible\s*\{[^}]*background:\s*var\(--brand-soft\);[^}]*outline:\s*2px solid var\(--brand\);/)
