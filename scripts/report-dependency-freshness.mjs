...
export function validatePolicy(policy, repo) {
  ...
  if (!Array.isArray(linkage.states) || !CROSS_REPOSITORY_LINKAGE_STATES.every((state) => linkage.states.includes(state))) {
    failures.push('cross-repository linkage must expose not-required, coupled, and script-only states');
  }
  if (linkage.defaultState !== 'not-required') failures.push('cross-repository linkage defaultState must be not-required');
